                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    @elseif($facility->status == 'đã duyệt') <span class="badge bg-success">Đang hoạt động</span>
                                    ...
                                    @else <span class="badge bg-light text-dark">{{ ucfirst($facility->status ?? 'Chưa rõ') }}</span>
                                    @endif
                                </td>
                                ...
                                <td class="text-end">
                                    ...
                                    @elseif($facility->status == 'đã duyệt')
                                        ...
                                        <form action="{{ route('admin.facility.suspend', $facility->facility_id) }}" method="POST"
                                              class="d-inline needs-confirmation" data-confirm-message="Bạn có chắc muốn tạm khóa cơ sở '{{ $facility->facility_name }}'?">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-outline-secondary" title="Tạm khóa hoạt động">
                                    ...
                                    @elseif($facility->status == 'tạm khóa')
                                        ...
                                        <form action="{{ route('admin.facility.activate', $facility->facility_id) }}" method="POST"
                                              class="d-inline needs-confirmation" data-confirm-message="Bạn có chắc muốn kích hoạt lại cơ sở '{{ $facility->facility_name }}'?">
                                            @csrf
                                    ...
                                    @elseif($facility->status == 'từ chối')
                                        ...
                                        <form action="{{ route('admin.facility.approve', $facility->facility_id) }}" method="POST" class="d-inline ms-1 needs-confirmation" data-confirm-message="Duyệt lại cơ sở này?"> 
                                            @csrf <button type="submit" class="btn btn-sm btn-outline-warning" title="Duyệt lại"><i class="bi bi-arrow-clockwise"></i> Duyệt Lại</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{-- Thông báo trạng thái --}}
                     @if($facility->status == 'chờ duyệt')
                        <div class="alert alert-warning d-flex align-items-center" role="alert"> <i class="bi bi-hourglass-split me-2"></i> <div> Yêu cầu này đang ở trạng thái <strong>Chờ Duyệt</strong>. </div> </div>
                    @elseif($facility->status == 'từ chối')
                         <div class="alert alert-danger d-flex align-items-center" role="alert"> <i class="bi bi-x-octagon-fill me-2"></i> <div> Yêu cầu này đã bị <strong>Từ Chối</strong>. </div> </div>
                    @endif
                ...
                @if($facility->status == 'chờ duyệt')
                    <div class="modal-footer justify-content-center"> 
                        <form action="{{ route('admin.facility.deny', $facility->facility_id) }}" method="POST" class="me-2"> 
                            @csrf 
                            <button type="submit" class="btn btn-danger"><i class="bi bi-x-circle me-1"></i> Từ chối Yêu cầu</button> 
                        </form>
                        <form action="{{ route('admin.facility.approve', $facility->facility_id) }}" method="POST"> 
                            @csrf 
                            <button type="submit" class="btn btn-success"><i class="bi bi-check2-circle me-1"></i> Chấp nhận Hoạt động</button> 
                        </form>
                    </div>
                @endif
            </div>
